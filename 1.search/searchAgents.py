...
class SearchAgent(Agent):
    ...
    def __init__(self, fn='depthFirstSearch', prob='ReplanningPositionSearchProblem', heuristic='manhattenHeuristic'):
...
# This class is used to positional search problems where replanning is required 
class ReplanningPositionSearchProblem(search.SearchProblem):

    def __init__(self, gameState, costFn=lambda x: 1, goal=(1, 1), start=None, warn=True, visualize=True):

        #Same as positional search problem but walls are not initialized 
        #Instead certain barriers are added which are anything that would block the cells
        self.walls = gameState.getWalls()
        self.startState = gameState.getPacmanPosition()
        if start != None:
        ...
        # Adding food location to the initial game state
        self.gameState = gameState
        self.goal = gameState.getFood().asList()[0]

        #Initializing the dimensions of the map being used
        ...
        self.visitedSuccessors = {}
        for state in self.getStates():
            self.visitedSuccessors[state] = False
    ...
    def cost(self, startState, goalState):
        # Similar to walls we will be returning 1 if there is no barrier else inf
        if startState in self.barriers or goalState in self.barriers:
            return float('inf')
        #  else it is 1
        return 1
    
    #Verify if there is any barrier
    def isbarrier(self, state):
        x, y = state
        if state in self.walls.asList():
            return True
        return False

    # update the barriers set when ever an barrier is found.
    def insertbarrier(self, barrier):
        self.barriers.add(barrier)

    # boundaries are not considered in replanning
    ...
    # This function is almost same as the positional search problem only change being states containing walls
    # are not excluded. while it is being excluded in the other search algorithms as the pacman would not know
    # the walls due to the constrains as specified in the project requirements
    def getSuccessors(self, state):
        successors = []
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            x,y = state
